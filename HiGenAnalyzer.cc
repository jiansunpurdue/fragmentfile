...
HiGenAnalyzer::HiGenAnalyzer(const edm::ParameterSet& iConfig)
{
  ...
  genParticleSrc_ = iConfig.getUntrackedParameter<edm::InputTag>("genpSrc",edm::InputTag("genParticles"));
...
//            if (p.daughter(idx)->pt()*p.daughter(idx)->eta()*p.daughter(idx)->phi() == pin.pt()*pin.eta()*pin.phi()) 
...
//            if (p.mother(idx)->pt()*p.mother(idx)->eta()*p.mother(idx)->phi() == pin.pt()*pin.eta()*pin.phi()) 
            if( fabs(p.mother(idx)->pt() - pin.pt()) < 0.001 && fabs(p.mother(idx)->eta() - pin.eta()) < 0.001 && fabs(p.mother(idx)->phi() - pin.phi()) < 0.001) 
...
HiGenAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(useHepMCProduct_){
    ...
    for(UInt_t i = 0; i < parts->size(); ++i){
     ...
     if( TMath::Abs(p.pdgId()) == 421 && p.pt() > 15.0 )
	 {
